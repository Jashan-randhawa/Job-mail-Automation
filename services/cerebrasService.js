...
// --- Retrying transient draft failures ---
// Two distinct things can go wrong generating a draft, and only one of them
// is worth retrying automatically:
//   1. Our own JSON.parse() fails on the model's output (rare — the model
//      wraps it in prose or fences despite instructions not to).
//   2. Groq's OWN server-side schema validation for `response_format:
//      { type: 'json_object' }` rejects the generation before we ever see
//      it, surfacing as an APIError with status 400 and a message like
//      "Failed to validate JSON. Please adjust your prompt. See
//      'failed_generation' for more details." This is a known occasional
//      flakiness of json_object mode, not something our own parsing can
//      fix — the fix is just asking again, since the same prompt often
//      succeeds on a second attempt.
// Plus the ordinary transient cases: rate limits (429) and Groq-side
// server errors (5xx). A single hiccup in any of these used to
// permanently fail the job (landing in `draft_failed`, needing a manual
// click of Retry) even though the very next attempt usually succeeds.
const MAX_DRAFT_ATTEMPTS = Number(process.env.GROQ_DRAFT_MAX_ATTEMPTS || 3);
const RETRYABLE_STATUS_CODES = new Set([429, 500, 502, 503, 504]);

function isRetryableDraftError(err) {
  const status = err?.status ?? err?.response?.status;
  if (status != null && RETRYABLE_STATUS_CODES.has(Number(status))) return true;
  const message = String(err?.message || err);
  return (
    /failed to validate json/i.test(message) || // Groq-side json_object schema rejection
    /was not valid json/i.test(message) || // our own JSON.parse failure (parseDraftResponse)
    /missing subject or body/i.test(message) ||
...
CHANNEL: EMAIL
Write the email using the exact structure below.
...
For email, write in exactly this structure. No extra paragraphs, no bullet lists.
...
// One attempt: reserve capacity, call Groq, parse, validate. Each attempt
// gets its own reservation/settle pair — a retried call is a brand new
// request against the rate limiter, not a reuse of the failed one's quota.
async function draftOnce(prompt, attempt, channel = 'email') {
  ...
  try {
    const completion = await getClient().chat.completions.create({
      ...
      messages: [
        ...
        // On a retry, reinforce the JSON-only instruction with a second,
        // terser message — cheap, and it's the one lever available for
        // Groq's own json_object-mode validation failures, which aren't
        // caused by anything wrong with our parsing.
        ...(attempt > 0
          ? [{ role: 'system', content: 'Return ONLY the JSON object described above. No markdown fences, no commentary before or after it.' }]
          : [])
      ],
      // Slightly lower temperature on retries: more deterministic output
      // is less likely to wander outside the schema a second time.
...
export async function extractConceptAndDraft(postText, { channel = 'email' } = {}) {
  const prompt = buildPrompt(postText, { channel });
  let lastError;
  for (let attempt = 0; attempt < MAX_DRAFT_ATTEMPTS; attempt++) {
    try {
      return await draftOnce(prompt, attempt, channel);
    } catch (err) {
      lastError = err;
      const attemptsLeft = MAX_DRAFT_ATTEMPTS - attempt - 1;
      if (attemptsLeft <= 0 || !isRetryableDraftError(err)) throw err;
      // Short, jittered backoff — this runs inside server.js's overall
      // DRAFT_TIMEOUT_MS budget for the job (withTimeout wraps the whole
      // call), so keep it brief rather than a long exponential climb.
      await sleep(400 * (attempt + 1) + Math.random() * 300);
    }
  }
  // Unreachable in practice (the loop always returns or throws above),
  // but keeps control flow explicit for anyone reading it.
  throw lastError;
}
