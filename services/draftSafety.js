...
export function checkDraftSafety(draft, { channel = 'email' } = {}) {
  ...
  const missingSubject = channel !== 'whatsapp' && (!draft?.subject || !draft.subject.trim());
  if (hasPlaceholder || tooShort || missingSubject) {
    return {
      rejected: true,
      reason: channel === 'whatsapp'
        ? 'Generated WhatsApp draft failed the safety check (placeholder text or too short). Nothing was sent.'
        : 'Generated email failed the safety check (placeholder text, missing subject, or too short). Nothing was sent.'
    };
  }
