...
import { sendWhatsappDirect } from '../services/whatsappService.js';
...
export default async function handler(req, res) {
  ...
  const { postText, recipientEmail, recipientPhone, channel = 'email' } = req.body || {};
  ...
  // Vercel can call the always-on WhatsApp service, but cannot host
  // whatsapp-web.js itself. For WhatsApp and Both, the dedicated service
  // owns the actual WhatsApp queue/pacing.
  if (normalizedChannel === 'whatsapp' || normalizedChannel === 'both') {
    ...
    if (normalizedChannel === 'both' && (!resolvedEmail || !EMAIL_RE.test(resolvedEmail))) {
    ...
    if (normalizedChannel === 'both' && (!resolvedEmail || !EMAIL_RE.test(resolvedEmail))) {
      return res.status(400).json({ error: 'For Both, enter a valid email or paste a post containing one.' });
    }
    ...
    emit({ phase: 'queued', channel: normalizedChannel, message: `Request received for ${normalizedChannel === 'both' ? 'email + WhatsApp' : 'WhatsApp'}.` });

    try {
      if (normalizedChannel === 'whatsapp') {
        emit({ phase: 'sending', channel: 'whatsapp', recipientPhone: resolvedPhone, message: 'Sending fixed greeting and resume PDF via WhatsApp.' });
        try {
          const result = await sendWhatsappDirect({ phone: resolvedPhone, attachResume: true });
          emit({
            phase: 'sent',
            channel: 'whatsapp',
          ...
          emit({ phase: 'send_failed', channel: 'whatsapp', recipientPhone: resolvedPhone, error: err?.message || String(err) });
        }
        return res.end();
      }

      // Channel is 'both': draft and send email, and send fixed greeting + resume via WhatsApp
      emit({ phase: 'drafting', channel: 'email', message: 'Generating personalized email draft.' });
      let emailDraft;
      try {
        emailDraft = await extractConceptAndDraft(postText.trim());
      } catch (err) {
        const message = err?.message || String(err);
        emit({ phase: 'draft_failed', channel: 'email', error: `Could not generate an email draft: ${message}` });
        return res.end();
      }

      const emailSafety = checkDraftSafety(emailDraft);
      if (emailSafety.rejected) {
        emit({ phase: 'rejected', channel: 'email', concept: emailDraft.concept, error: emailSafety.reason });
        return res.end();
      }

      emit({ phase: 'sending', channel: 'both', message: 'Sending email via SMTP and WhatsApp greeting directly.' });

      const whatsappPromise = sendWhatsappDirect({ phone: resolvedPhone, attachResume: true })
        .then((accepted) => ({ ok: true, accepted }))
        .catch((err) => ({ ok: false, error: err }));

      const emailPromise = sendOutreachEmail({ to: resolvedEmail, subject: emailDraft.subject, body: emailDraft.body })
        .then(() => ({ ok: true }))
        .catch((err) => ({ ok: false, error: err }));
      ...
      if (whatsappResult.ok) {
        emit({
          ...
          channel: 'whatsapp',
          recipientPhone: resolvedPhone,
          attachmentSent: whatsappResult.accepted?.attachmentSent,
          previouslySent: whatsappResult.accepted?.previouslySent,
          message: 'WhatsApp greeting and resume sent.'
        });
      } else {
        emit({ phase: 'send_failed', channel: 'whatsapp', error: whatsappResult.error?.message || String(whatsappResult.error) });
      }

      if (emailResult.ok) {
        emit({ phase: 'sent', channel: 'email', recipientEmail: resolvedEmail, emailAutoDetected, subject: emailDraft.subject, concept: emailDraft.concept, message: 'Email sent.' });
      } else {
        emit({ phase: 'send_failed', channel: 'email', error: `Email send failed: ${emailResult.error?.message || String(emailResult.error)}` });
      }

      return res.end();
    } catch (err) {
      emit({ phase: 'send_failed', channel: normalizedChannel, error: err?.message || String(err) });
      return res.end();
    }
  }
