...
import { sendWhatsappDirect, getWhatsappStatus, getWhatsappQr } from './services/whatsappService.js';
...
export const queueConfig = {
  ...
  whatsappJobTimeoutMs: Number(process.env.WHATSAPP_JOB_TIMEOUT_MS || 10 * 60_000),
  whatsappPollMs: Number(process.env.WHATSAPP_POLL_MS || 3_000)
};
...
let whatsappWorkerPromise = null;
let activeWhatsappJobId = null;
const whatsappQueuedJobIds = [];
let whatsappQueueHead = 0;
let lastSendAt = 0;
...
export function assertQueueInvariants({ allowStoppedWorker = false } = {}) {
  ...
  if (activeJobs.filter((j) => (j.channel || 'email') === 'email').length > 1) {
    problems.push(`more than one active email job: ${activeJobs.filter((j) => (j.channel || 'email') === 'email').map((j) => j.id).join(',')}`);
  }
  if (activeJobs.filter((j) => j.channel === 'whatsapp').length > 1) {
    problems.push(`more than one active WhatsApp job: ${activeJobs.filter((j) => j.channel === 'whatsapp').map((j) => j.id).join(',')}`);
  }
  if (activeJobId && queuedJobIds.indexOf(activeJobId, queueHead) !== -1) problems.push(`active email job is also queued: ${activeJobId}`);
  if (activeWhatsappJobId && whatsappQueuedJobIds.indexOf(activeWhatsappJobId, whatsappQueueHead) !== -1) problems.push(`active WhatsApp job is also queued: ${activeWhatsappJobId}`);
  if (activeJobId && !jobs.has(activeJobId)) problems.push(`activeJobId ${activeJobId} references a nonexistent job`);
  if (activeWhatsappJobId && !jobs.has(activeWhatsappJobId)) problems.push(`activeWhatsappJobId ${activeWhatsappJobId} references a nonexistent job`);
  const activeEmailJobs = activeJobs.filter((j) => (j.channel || 'email') === 'email');
  const activeWaJobs = activeJobs.filter((j) => j.channel === 'whatsapp');
  if (activeEmailJobs.length === 1 && activeEmailJobs[0].id !== activeJobId) problems.push(`email job ${activeEmailJobs[0].id} is active but activeJobId is ${activeJobId || 'null'}`);
  if (activeWaJobs.length === 1 && activeWaJobs[0].id !== activeWhatsappJobId) problems.push(`WhatsApp job ${activeWaJobs[0].id} is active but activeWhatsappJobId is ${activeWhatsappJobId || 'null'}`);
  if (activeEmailJobs.length === 0 && activeJobId) problems.push(`activeJobId ${activeJobId} is set but no email job is active`);
  if (activeWaJobs.length === 0 && activeWhatsappJobId) problems.push(`activeWhatsappJobId ${activeWhatsappJobId} is set but no WhatsApp job is active`);
...
function makeJob(postText, recipientEmail, { channel = 'email', phone = null, batch = true } = {}) {
  const id = crypto.randomUUID();
  const ts = now();
  jobs.set(id, { id, status: 'queued', postText, recipientEmail, phone, channel, batchId: null, createdAt: ts, updatedAt: ts, startedAt: null, draftedAt: null, waitingAt: null, sendingAt: null, completedAt: null, plannedSendAt: null, result: null, error: null, lastErrorAt: null, retryCount: 0, claimedBy: null, claimedAt: null, lockUntil: null, sendAttemptStartedAt: null, queueSeq: nextQueueSeq++, events: [{ at: ts, phase: 'queued', message: 'Queued for processing.' }] });
  if (channel === 'whatsapp') {
    whatsappQueuedJobIds.push(id);
  } else {
    queuedJobIds.push(id);
    if (batch) assignJobToBatch(id);
  }
...
function estimateEtaSeconds(job) {
  if (job.channel === 'whatsapp') {
    if (job.status === 'queued') return Math.max(0, (whatsappQueuePosition(job.id) || 1) - 1) * Math.round(queueConfig.estimatedDraftSeconds);
    if (job.status === 'waiting') return Math.round(queueConfig.whatsappPollMs / 1000);
    if (job.status === 'sending') return Math.max(0, Math.round(queueConfig.whatsappPollMs / 1000));
  }
...
function serializeJob(job, { includeDraft = false } = {}) {
  ...
  const body = { id: job.id, status: job.status, state: job.status, channel: job.channel || 'email', phone: job.phone || null, recipientEmail: job.recipientEmail, batchId: job.batchId, createdAt: asIso(job.createdAt), updatedAt: asIso(job.updatedAt), startedAt: asIso(job.startedAt), draftedAt: asIso(job.draftedAt), waitingAt: asIso(job.waitingAt), sendingAt: asIso(job.sendingAt), completedAt: asIso(job.completedAt), plannedSendAt: asIso(job.plannedSendAt), lastErrorAt: asIso(job.lastErrorAt), inFlight: activeJobId === job.id, active: activeJobId === job.id, position: (job.channel === 'whatsapp' ? (activeWhatsappJobId === job.id ? 0 : whatsappQueuePosition(job.id)) : (activeJobId === job.id ? 0 : queuePosition(job.id))), etaSeconds: eta, error: job.error, retryCount: job.retryCount || 0, canRetry: FAILURE_STATUSES.has(job.status), claimedBy: job.claimedBy, claimedAt: asIso(job.claimedAt), lockUntil: asIso(job.lockUntil), events: job.events.map((e) => ({ ...e, at: asIso(e.at) })) };
...
function recoverStaleJobs() {
  ...
  for (const job of jobs.values()) {
    if (ACTIVE_STATUSES.has(job.status) && job.lockUntil && job.lockUntil < cutoff) {
      ...
      if (activeWhatsappJobId === job.id) activeWhatsappJobId = null;
...
function snapshotState() {
  return {
    ...
    whatsappQueuedJobIds: whatsappQueuedJobIds.slice(whatsappQueueHead),
...
function restoreState(state) {
  if (!state) return;
  jobs.clear(); queuedJobIds.splice(0); whatsappQueuedJobIds.splice(0); batches.clear(); pendingBatchIds.splice(0);
  ...
  whatsappQueueHead = 0;
  for (const id of state.whatsappQueuedJobIds || []) whatsappQueuedJobIds.push(id);
...
function startStaleSweep() {
  ...
  staleSweepTimer = setInterval(() => {
    ...
    if (whatsappQueueLength() > 0) runWhatsappWorker(); // promoteBatches() itself never calls runWorker() — see its comment; this is the wake-up for the "nothing was actively draining" case
...
async function processWhatsappJob(job) {
  transitionJob(job, 'sending', { message: 'Sending WhatsApp fixed greeting and resume PDF.' });
  try {
    const result = await withTimeout(
      sendWhatsappDirect({ phone: job.phone, attachResume: true }),
      queueConfig.whatsappJobTimeoutMs,
      'WhatsApp direct send'
    );
    transitionJob(job, 'sent', {
      completedAt: now(),
      result: {
        channel: 'whatsapp',
        phone: job.phone,
        attachmentSent: result.attachmentSent,
        previouslySent: result.previouslySent
      },
      message: 'WhatsApp greeting and resume sent successfully.'
    });
  } catch (err) {
    const message = normalizeError(err).message;
    const outcome = /timeout/i.test(message) ? 'send_unknown' : 'send_failed';
    failActiveJob(job, outcome, `WhatsApp sending failed: ${message}`);
  }
}

async function drainWhatsappQueue(workerId, myGeneration) {
  console.log(`[whatsapp-worker] ${workerId} started with ${whatsappQueueLength()} queued job(s).`);
  while (true) {
    if (myGeneration !== generation) return;
    const jobId = whatsappQueuedJobIds[whatsappQueueHead];
    if (jobId === undefined) break;
    const job = jobs.get(jobId);
    if (!job) { advanceWhatsappQueueHead(); continue; }
    if (job.status !== 'queued') { advanceWhatsappQueueHead(); continue; }
    activeWhatsappJobId = jobId;
    transitionJob(job, 'processing', {
      claimedBy: workerId,
      claimedAt: now(),
      lockUntil: now() + queueConfig.whatsappJobTimeoutMs,
      message: 'Safely claimed by WhatsApp worker.'
    });
    advanceWhatsappQueueHead();
    job.startedAt = job.startedAt || now();
    try {
      await processWhatsappJob(job);
    } catch (err) {
      failActiveJob(job, job.status === 'sending' ? 'send_unknown' : 'draft_failed', `Unexpected WhatsApp worker error: ${normalizeError(err).message}`);
    } finally {
      if (activeWhatsappJobId === jobId) activeWhatsappJobId = null;
      job.claimedBy = null;
      job.lockUntil = null;
      schedulePersist();
    }
  }
  console.log(`[whatsapp-worker] ${workerId} queue drained.`);
}

function whatsappQueueLength() { return whatsappQueuedJobIds.length - whatsappQueueHead; }
function whatsappQueuePosition(jobId) {
  const idx = whatsappQueuedJobIds.indexOf(jobId, whatsappQueueHead);
  return idx === -1 ? null : idx - whatsappQueueHead + 1;
}
function advanceWhatsappQueueHead() {
  whatsappQueueHead += 1;
  if (whatsappQueueHead > 64 && whatsappQueueHead * 2 > whatsappQueuedJobIds.length) {
    whatsappQueuedJobIds.splice(0, whatsappQueueHead);
    whatsappQueueHead = 0;
  }
}
function runWhatsappWorker() {
  if (whatsappWorkerPromise) return whatsappWorkerPromise;
  const workerId = `whatsapp-worker-${++workerSeq}`;
  const myGeneration = generation;
  whatsappWorkerPromise = drainWhatsappQueue(workerId, myGeneration)
    .catch((err) => console.error('[whatsapp-worker] fatal drain error:', err))
    .finally(() => {
      if (myGeneration !== generation) return;
      whatsappWorkerPromise = null;
      if (whatsappQueueLength() > 0) runWhatsappWorker();
    });
  return whatsappWorkerPromise;
}
...
app.post('/api/send-outreach', (req, res) => {
  ...
  if (!['email', 'whatsapp', 'both'].includes(normalizedChannel)) {
    return res.status(400).json({ error: 'Channel must be email, whatsapp, or both.' });
  }

  let resolvedEmail = recipientEmail && String(recipientEmail).trim();
  let emailAutoDetected = false;
  if (normalizedChannel !== 'whatsapp' && (!resolvedEmail || !EMAIL_RE.test(resolvedEmail))) {
  ...
  const resolvedPhone = recipientPhone ? String(recipientPhone).trim() : '';
  if ((normalizedChannel === 'whatsapp' || normalizedChannel === 'both') && !/^\+?[1-9]\d{7,14}$/.test(resolvedPhone)) {
    return res.status(400).json({ error: 'Enter a valid WhatsApp number in international format, e.g. +919876543210.' });
  }

  const jobIds = [];
  if (normalizedChannel === 'email' || normalizedChannel === 'both') {
    jobIds.push(makeJob(postText.trim(), resolvedEmail, { channel: 'email' }));
  }
  if (normalizedChannel === 'whatsapp' || normalizedChannel === 'both') {
    jobIds.push(makeJob(postText.trim(), normalizedChannel === 'both' ? resolvedEmail : null, {
      channel: 'whatsapp',
      phone: resolvedPhone,
      batch: false
    }));
  ...
  const serialized = jobIds.map((id) => serializeJob(jobs.get(id)));
  if (jobIds.some((id) => jobs.get(id)?.channel === 'email')) runWorker();
  if (jobIds.some((id) => jobs.get(id)?.channel === 'whatsapp')) runWhatsappWorker();
  ...
  res.status(202).json({
    ...
    recipientEmail: resolvedEmail || null,
...
app.get('/api/whatsapp/status', async (_req, res) => {
  try {
    ...
    res.json({
      ...status,
      mainQueue: {
        running: Boolean(whatsappWorkerPromise),
        activeJobId: activeWhatsappJobId,
        queuedCount: whatsappQueueLength()
      }
    });
  } catch (err) {
    res.status(503).json({ connected: false, status: 'unavailable', error: normalizeError(err).message });
  }
});
...
app.get('/api/whatsapp/qr', async (_req, res) => {
  try {
    ...
    res.status(503).json({ error: normalizeError(err).message });
  }
});
...
app.post('/api/jobs/:jobId/retry', (req, res) => {
  ...
  if (job.channel === 'whatsapp') {
    whatsappQueuedJobIds.push(job.id);
    transitionJob(job, 'queued', { message: `Re-queued in the WhatsApp queue for retry attempt ${job.retryCount + 1}.` });
    runWhatsappWorker();
  } else {
  ...
  schedulePersist();
  res.status(202).json({ jobId: job.id, status: 'queued', position: job.channel === 'whatsapp' ? whatsappQueuePosition(job.id) : queuePosition(job.id) || 1, etaSeconds: estimateEtaSeconds(job) });
});
...
app.get('/api/jobs', (_req, res) => {
  ...
  res.json({ worker: { running: Boolean(workerPromise), activeJobId, currentJobId: activeJobId, queuedCount: queueLength(), whatsappQueuedCount: whatsappQueueLength(), whatsappActiveJobId: activeWhatsappJobId, queueIds: queuedJobIds.slice(queueHead), lastSendAt: asIso(lastSendAt), nextEligibleSendAt: lastSendAt || claimedSendSlotAt ? asIso(nextEligibleSendAt()) : null }, batches: serializeBatchSummary(), jobs: jobsList });
});

export const __queueTest = {
  jobs, queuedJobIds, whatsappQueuedJobIds,
  ...
  reset() {
    ...
    jobs.clear(); queuedJobIds.splice(0); queueHead = 0; activeJobId = null; workerPromise = null; whatsappQueuedJobIds.splice(0); whatsappQueueHead = 0; activeWhatsappJobId = null; whatsappWorkerPromise = null; lastSendAt = 0; claimedSendSlotAt = 0; workerSeq = 0; nextQueueSeq = 0; dequeueSeq = 0;
  ...
  getDiagnostics() { return { workerPromise, whatsappWorkerPromise, activeJobId, activeWhatsappJobId, queueIds: [...queuedJobIds], whatsappQueueIds: [...whatsappQueuedJobIds], lastSendAt, claimedSendSlotAt, jobs: Array.from(jobs.values()), batches: Array.from(batches.values()), pendingBatchIds: [...pendingBatchIds], assemblingBatchId: assemblingBatch?.id || null }; }
};
