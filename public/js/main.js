...
(function engine(){
  ...
  const NAV_SCROLL_ENTER = 80;
  const NAV_SCROLL_EXIT = 40;
  ...
  function tick(){
    ...
    // Navigation bar scroll state, folded into this same rAF-batched tick
    // (see fix note near onScroll below) with separate enter/exit
    // thresholds so it can't flap back and forth near a single line.
    if (topNav){
      if (!navScrolled && y > NAV_SCROLL_ENTER) navScrolled = true;
      else if (navScrolled && y < NAV_SCROLL_EXIT) navScrolled = false;
  ...
  // Navigation bar scroll effect: 'scrolled' state now lives inside tick()
  // above, not here. It used to toggle straight from this raw 'scroll'
  // event with a single hard threshold (80px) — scroll events can fire
  // dozens of times per frame, so momentum/rubber-band scrolling hovering
  // near that line kept re-triggering the class (and restarting its CSS
  // transition) many times per frame. Combined with a dark-mode selector
  // bug elsewhere (see style.css), that repeated restart is what produced
  // the visible color-fade/flicker. Batching it into the same rAF tick as
  // every other scroll read, with separate enter/exit thresholds, fixes
  // both the restart-storm and the flapping at the boundary.
  function onScroll(){
...
const whatsappStatusHintEl = document.getElementById('whatsappStatusHint');
const emailHintEl = document.getElementById('recipientEmailHint');
const history = []; // most recent first; each entry also carries channel/phone for WhatsApp jobs

/* ================= CHANNEL SELECTION ================= */
const channelCopy = {
  email: 'Email outreach drafts personalized emails via AI and sends through Gmail SMTP.',
  whatsapp: 'WhatsApp outreach sends a fixed greeting message and your resume PDF directly on demand.',
  both: 'Email is drafted with AI; WhatsApp sends your fixed greeting and resume directly.'
};
function syncChannelUi(){
  const channel = channelEl?.value || 'email';
  channelOptions.forEach((button) => button.classList.toggle('active', button.dataset.channel === channel));
  if (channelHintEl) channelHintEl.textContent = channelCopy[channel];
  const needsEmail = channel === 'email' || channel === 'both';
  const needsPhone = channel === 'whatsapp' || channel === 'both';
  const emailWrap = document.getElementById('emailRecipientWrap');
  const phoneWrap = document.getElementById('phoneRecipientWrap');
  if (emailWrap) emailWrap.hidden = !needsEmail;
  if (phoneWrap) phoneWrap.hidden = !needsPhone;
  if (recipientEmailEl) recipientEmailEl.required = needsEmail;
  if (recipientPhoneEl) recipientPhoneEl.required = needsPhone;
}
channelOptions.forEach((button) => button.addEventListener('click', () => {
  if (!channelEl) return;
  channelEl.value = button.dataset.channel;
  syncChannelUi();
  if (button.dataset.channel !== 'email') refreshWhatsappStatus();
}));
syncChannelUi();

const waSettings = {
  dot: document.getElementById('waSettingsDot'),
  status: document.getElementById('waSettingsStatus'),
  meta: document.getElementById('waSettingsMeta'),
  queued: document.getElementById('waQueuedCount'),
  sent: document.getElementById('waSentToday'),
  cap: document.getElementById('waDailyCap'),
  refresh: document.getElementById('refreshWhatsappBtn'),
  showQr: document.getElementById('showWhatsappQrBtn'),
  qrPanel: document.getElementById('waQrPanel'),
  qrImage: document.getElementById('waQrImage'),
  qrEmpty: document.getElementById('waQrEmpty'),
  qrMessage: document.getElementById('waQrMessage')
};

function renderWhatsappSettings(data, available = true){
  if (!waSettings.status) return;
  const connected = Boolean(available && data?.connected);
  const needsRescan = Boolean(data?.needsRescan || data?.status === 'auth_failure' || data?.status === 'disconnected');
  waSettings.dot.className = `wa-status-dot ${connected ? 'is-connected' : needsRescan ? 'is-warning' : 'is-offline'}`;
  waSettings.status.textContent = connected ? 'WhatsApp connected' : needsRescan ? 'Re-scan required' : available ? `WhatsApp ${data?.status || 'not ready'}` : 'WhatsApp service unavailable';
  waSettings.meta.textContent = connected
    ? `Ready since ${data.readyAt ? new Date(data.readyAt).toLocaleString() : 'now'}.`
    : needsRescan ? 'The session was logged out or authentication expired.' : (data?.error || 'Start the persistent WhatsApp service and pair your account.');
  if (waSettings.sent) waSettings.sent.textContent = data?.totalSent ?? data?.sentToday ?? '0';
}
...
async function loadWhatsappSettings(){
  try {
    ...
    renderWhatsappSettings(data, res.ok);
    return data;
  } catch (err) {
    renderWhatsappSettings({ error: 'Could not reach the WhatsApp service.' }, false);
    return null;
  }
}

async function showWhatsappQr(){
  if (!waSettings.qrPanel) return;
  waSettings.qrPanel.hidden = false;
  waSettings.qrImage.hidden = true;
  waSettings.qrEmpty.hidden = false;
  waSettings.qrMessage.textContent = 'Loading the current pairing state…';
  try {
    const res = await fetch('/api/whatsapp/qr', { cache: 'no-store' });
    const data = await res.json().catch(() => ({}));
    if (data.qrDataUrl) {
      waSettings.qrImage.src = data.qrDataUrl;
      waSettings.qrImage.hidden = false;
      waSettings.qrEmpty.hidden = true;
      waSettings.qrMessage.textContent = 'Open WhatsApp → Linked Devices → Link a device, then scan this code.';
    } else if (data.status === 'connected') {
      waSettings.qrMessage.textContent = 'This WhatsApp session is already connected. No pairing QR is needed.';
      waSettings.qrEmpty.textContent = 'Already connected';
    } else {
      waSettings.qrMessage.textContent = `No pairing QR is ready yet (status: ${data.status || 'unknown'}). Refresh in a moment.`;
      waSettings.qrEmpty.textContent = 'Waiting for QR…';
    }
  } catch (err) {
    waSettings.qrMessage.textContent = 'Could not load the pairing QR. Check the WhatsApp service deployment and API key.';
    waSettings.qrEmpty.textContent = 'QR unavailable';
  }
}
waSettings.refresh?.addEventListener('click', loadWhatsappSettings);
waSettings.showQr?.addEventListener('click', showWhatsappQr);
if (waSettings.status) loadWhatsappSettings();

async function refreshWhatsappStatus(){
  if (!whatsappStatusHintEl) return;
  whatsappStatusHintEl.textContent = 'Checking WhatsApp connection…';
  whatsappStatusHintEl.className = 'field-hint field-hint--muted';
  try {
    const res = await fetch('/api/whatsapp/status', { cache: 'no-store' });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data.status === 'unavailable'){
      whatsappStatusHintEl.textContent = data.error || 'WhatsApp service unavailable. Set WHATSAPP_SERVICE_URL and WHATSAPP_API_KEY.';
      whatsappStatusHintEl.className = 'field-hint field-hint--error';
      return;
    }
    if (data.connected){
      whatsappStatusHintEl.textContent = 'WhatsApp connected · Ready for direct send with resume attachment.';
      whatsappStatusHintEl.className = 'field-hint field-hint--success';
    } else {
      whatsappStatusHintEl.textContent = data.needsRescan ? 'WhatsApp needs a re-scan. Open /api/whatsapp/qr on the service to pair again.' : `WhatsApp status: ${data.status}.`;
      whatsappStatusHintEl.className = 'field-hint field-hint--error';
    }
  } catch (_err){
    whatsappStatusHintEl.textContent = 'Could not reach the WhatsApp service.';
    whatsappStatusHintEl.className = 'field-hint field-hint--error';
  }
}
...
function renderHistory(){
  jobsEl.innerHTML = history.map((entry) => {
    ...
    const channel = `<div class="job-meta-row"><span class="job-meta-label">Channel</span> ${escapeHtml(entry.channel || 'email')}</div>`;
    ...
    return `
      <article class="ledger-row">
        ...
        <div class="ledger-row-main">
          <div class="job-recipient">${escapeHtml(entry.channel === 'whatsapp' ? (entry.recipientPhone || entry.phone || 'unknown') : (entry.recipientEmail || 'unknown'))}</div>
          ${channel}${concept}${subject}${eta}${error}
        </div>
        ${retryBtn}
      </article>`;
  }).join('');
...
async function runQueue(){
  ...
  while (submitQueue.length > 0){
    ...
    const entry = await runSubmission(job.postText, job.recipientEmail, { recipientPhone: job.recipientPhone, channel: job.channel, retryOf: job.retryOf });
...
function pollWhatsappJob(remoteJobId, mainJobId, recipientPhone){
  const tick = async () => {
    try {
      const res = await fetch(`/api/whatsapp-status?jobId=${encodeURIComponent(remoteJobId)}`);
      if (!res.ok) return;
      const remote = await res.json();
      const i = history.findIndex((h) => h.id === mainJobId);
      if (i >= 0){
        history[i] = { ...history[i], status: remote.status === 'sent' ? 'sent' : remote.status === 'failed' ? 'send_failed' : 'sending', recipientPhone, whatsappJobId: remoteJobId };
        renderHistory();
      }
      if (remote.status === 'sent' || remote.status === 'failed') clearInterval(timer);
    } catch (_err) {}
  };
  tick();
  const timer = setInterval(tick, 3000);
  setTimeout(() => clearInterval(timer), 15 * 60_000);
}
...
async function runSubmission(postText, recipientEmail, { recipientPhone, channel = 'email', retryOf } = {}){
  ...
  let entry = { id: crypto.randomUUID(), status: 'queued', recipientEmail, recipientPhone, channel, postText };

  try {
    let res = await fetch('/api/send-outreach', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postText, recipientEmail, recipientPhone, channel }) });
    ...
    for (let attempt = 0; res.status === 429 && attempt < 5; attempt++){
      ...
      res = await fetch('/api/send-outreach', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ postText, recipientEmail, recipientPhone, channel }) });
    }
    ...
    if (!res.ok){
      ...
      await streamOutreach(res, (event) => {
        entry = { ...entry, ...event, id: event.jobId || entry.id, status: event.phase, pathA: false, channel };
        ...
        if (channel === 'whatsapp' && event.whatsappJobId) {
          pollWhatsappJob(event.whatsappJobId, entry.id, recipientPhone);
        }
      });
    } else {
      ...
      entry = { ...entry, ...data, id: data.jobId || entry.id, status: data.status || 'queued', pathA: true, channel: data.channel || channel, recipientPhone: data.recipientPhone || recipientPhone };
      ...
      // "Both" creates one independent job per channel so the email and
      // WhatsApp workers can progress in parallel.
      if (Array.isArray(data.jobIds) && data.jobIds.length > 1){
        const extraIds = data.jobIds.slice(1);
        for (const extraId of extraIds){
          const extraChannel = channel === 'both' && extraId === data.jobIds.find((id) => id !== data.jobId) ? 'whatsapp' : channel;
          const extra = {
            id: extraId,
            status: 'queued',
            recipientEmail: extraChannel === 'email' ? recipientEmail : null,
            recipientPhone: extraChannel === 'whatsapp' ? recipientPhone : null,
            channel: extraChannel,
            postText,
            pathA: true
          };
          history.unshift(extra);
          pollJobStatus(extraId);
        }
        renderHistory();
      }
    }
...
async function submitOutreach(postText, recipientEmail, { recipientPhone, channel = 'email', retryOf } = {}){
  submitQueue.push({ postText, recipientEmail, recipientPhone, channel, retryOf });
  if (!retryOf){
    const keepChannel = channelEl?.value || channel;
    form.reset();
    if (channelEl) channelEl.value = keepChannel;
    syncChannelUi();
    recipientEditedByUser = false; lastAutoFilledEmail = ''; setEmailHint('');
  }
  updateQueueLabel();
  runQueue();
}
...
async function retryJob(entry){
  if (!entry.pathA){
    ...
    // comment).
    submitOutreach(entry.postText, entry.recipientEmail, { recipientPhone: entry.recipientPhone, channel: entry.channel, retryOf: entry.id });
    return;
  }
  try {
...
form.addEventListener('submit', (e) => {
  ...
  submitOutreach(postText, recipientEmail, { recipientPhone, channel });
});
renderHistory();
